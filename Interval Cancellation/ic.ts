type sFn = (...argsArr: number[]) => void;
type cnFn = (fn: sFn, args: number[], t: number) => () => void;

var cancellable: cnFn = function (fn, args, t) {
  let mInterval: ReturnType<typeof setInterval>;
  let lastArrL: number = 0;
  fn(...args);
  let second: number = 1;
  const execute = (): void => {
    lastArrL += 1;
  ...
  mInterval = setInterval(execute, t);
